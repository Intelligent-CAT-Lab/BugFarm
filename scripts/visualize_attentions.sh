...
function prompt() {
    echo;
    echo "Syntax: bash scripts/visualize_attentions.sh LOG_FILE_NAME MODEL_NAME NUM_LAYERS THRESHOLD";
...
do
    python3 visualize_attention.py --project_name $project --log_file $LOG_FILE_NAME --model_type $MODEL_NAME --num_layers $NUM_LAYERS --threshold $THRESHOLD;
done
