...
function prompt() {
    echo;
    echo "Syntax: bash scripts/extract_attentions.sh LOG_FILE_NAME MODEL_NAME NUM_LAYERS NUM_WORKERS";
...
do
    python3 attention_extractor.py --project_name $project --log_file $LOG_FILE_NAME --model_type $MODEL_NAME --num_layers $NUM_LAYERS --num_workers $NUM_WORKERS;
done
